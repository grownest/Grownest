...
class FAQController {
    ...
    initialize() {
        ...
        if (this.initialized) {
            console.log('FAQController already initialized');
            return;
        }

        this.faqItems = document.querySelectorAll('.faq-item');
        // Check if FAQ items are available
        if (this.faqItems.length === 0 && this.retryCount < this.maxRetries) {
            this.retryCount++;
            console.log(`FAQ: No items found, retrying (${this.retryCount}/${this.maxRetries})...`);
            setTimeout(() => this.initialize(), 200);
            return;
        }

        console.log('FAQ initialization:', {
            faqItems: this.faqItems.length,
            retryCount: this.retryCount,
            elements: Array.from(this.faqItems).map((item, index) => ({
                index,
                hasQuestion: !!item.querySelector('.faq-question'),
                hasAnswer: !!item.querySelector('.faq-answer')
            }))
        });
        if (this.faqItems.length > 0) {
            ...
            console.error('FAQ Controller: No FAQ items found after retries!');
        }
    }

    init() {
        this.setupFAQHandlers();
        console.log('FAQ controller initialized successfully with', this.faqItems.length, 'items');
    }

    setupFAQHandlers() {
        // Helper function to add event listeners safely
        const addEventListeners = (element, type, handler) => {
            try {
                element.removeEventListener(type, handler);
                element.addEventListener(type, handler, { passive: false });
                return true;
            } catch (error) {
                console.error(`Failed to add ${type} listener:`, error);
                return false;
            }
        };
        this.faqItems.forEach((item, index) => {
            ...
            if (question && answer) {
                console.log(`Setting up FAQ item ${index + 1}:`, {
                    ...
                    hasAnswer: !!answer,
                    initialState: question.getAttribute('aria-expanded')
                });
                ...
                // Make sure the question is focusable
                question.setAttribute('tabindex', '0');
                question.style.cursor = 'pointer';
                question.disabled = false;

                // **Mistake was here: The existing click and key handlers were not set up to properly call the toggle function.**
                // Corrected handlers:
                question.addEventListener('click', (e) => {
                    e.preventDefault();
                    this.toggleFAQ(question, answer, index);
                });
                ...
                question.addEventListener('mouseenter', () => {
                    try {
                        question.style.transform = 'translateY(-1px)';
                    } catch (error) {
                ...
                question.addEventListener('mouseleave', () => {
                    try {
                        ...
                        console.error('Hover reset error:', error);
                    }
                });
            } else {
                console.error(`FAQ item ${index + 1} missing required elements:`, {
        ...
        console.log('FAQ handlers setup completed for', this.faqItems.length, 'items');
    }

    toggleFAQ(question, answer, index) {
        const isOpen = question.getAttribute('aria-expanded') === 'true';
        console.log(`Toggling FAQ ${index + 1}:`, {
            currentState: isOpen ? 'open' : 'closed',
            newState: isOpen ? 'closing' : 'opening'
        });
        ...
        if (isOpen) {
            ...
            if (content) {
                ...
                console.log(`FAQ ${index + 1} opened with height:`, contentHeight + 40);
            } else {
                answer.style.maxHeight = '200px';
                answer.style.opacity = '1';
                
                console.log(`FAQ ${index + 1} opened with default height`);
            }
        }
        
        // Update visual state of the question (for icon rotation)
        ...
        const stateText = isOpen ? 'collapsed' : 'expanded';
        this.announceToScreenReader(`${questionText} ${stateText}`);
    }
...
class ContactFormController {
    ...
    init() {
        ...
        this.setTimestamp();
        console.log('Contact form controller initialized successfully');
    }
    ...
    setSubmitLoading(loading) {
        if (this.submitBtn) {
            ...
            const loadingSpinner = this.submitBtn.querySelector('.submit-loading');
            
            if (text && loadingSpinner) {
